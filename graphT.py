...
class Graph:
    ...
    def reverse_bfs(self, initial_node):
     queue = deque([(initial_node, [initial_node])])  # Queue stores tuples of (node, path_to_node)
     visited = set([initial_node])  # Keep track of visited nodes to prevent revisiting

     while queue:
        current_node, path = queue.popleft()  # Current node being explored and the path to reach it

        # If the current node is a goal, return the path to it
        if self.is_goal(current_node):
            return path

        # Explore all neighbors of the current node
        for neighbor in self.get_neighbors(current_node):
            if neighbor not in visited:  # Only consider unvisited neighbors to avoid cycles
                visited.add(neighbor)  # Mark this neighbor as visited
                # Add the neighbor to the queue with the updated path including this neighbor
                queue.append((neighbor, path + [neighbor]))

     return "No solution"  # Return this if no goal node can be reached from the initial node
    ...
    def best_move(self, initial_node):
     queue = deque([(initial_node, [])])  # Queue stores tuples of (node, path_to_node)
     visited = set([initial_node])  # Keep track of visited nodes to prevent revisits

     while queue:
        current_node, path = queue.popleft()  # Current node being explored and the path to reach it

        # If the current node is a goal, return the first move towards this goal node
        if self.is_goal(current_node):
            if path:
                return path[0]  # Return the first move that leads towards the goal
            else:
                return current_node  # If the goal is the initial node, return it directly

        # Explore all neighbors of the current node
        for neighbor in self.get_neighbors(current_node):
            if neighbor not in visited:
                visited.add(neighbor)  # Mark this neighbor as visited
                # Append this neighbor to the queue with the updated path including this neighbor
                queue.append((neighbor, path + [neighbor]))

     return "No solution"  # If the loop completes without finding a goal node
